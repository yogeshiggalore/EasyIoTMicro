...
#define INPUT_TOUCH_1   15
#define INPUT_TOUCH_2   12
#define INPUT_TOUCH_3   14
#define INPUT_TOUCH_4   27
...
void read_touch(uint8_t ui8Touch);
...
void loop() {
	...
	if(bleMouse.isConnected()){
		...
		//read_touch(TH3);
		//read_touch(TH4);
		Serial.print(touchRead(INPUT_TOUCH_1));
		Serial.print(" ");
		Serial.print(touchRead(INPUT_TOUCH_2));
		Serial.print(" ");
		Serial.print(touchRead(INPUT_TOUCH_3));
		Serial.print(" ");
		Serial.print(touchRead(INPUT_TOUCH_4));
		Serial.println(" ");
		delay(100);
	}
}
...
void read_touch(uint8_t ui8Touch){
	...
	if(ui8Touch == TH1){
		if(touchRead(INPUT_TOUCH_1) < 20){
			bleMouse.move(0,0,0,-1);
			delay(10);
		}
	}else if(ui8Touch == TH2){
		if(touchRead(INPUT_TOUCH_2) < 20){
			bleMouse.move(0,0,0,1);
			delay(10);
		}
	}else if(ui8Touch == TH3){
		if(touchRead(INPUT_TOUCH_3) < 20){
			bleMouse.move(0,0,1);
			delay(10);
		}
	}else if(ui8Touch == TH4){
		if(touchRead(INPUT_TOUCH_4) < 20){
			bleMouse.move(0,0,-1);
			delay(10);
		}
	}else{
